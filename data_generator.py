...
# Dao động từ 0.5 km đến 10.0 km
distances = np.round(np.random.uniform(0.5, 10.0, n_samples), 2)
...
# Để trọ cách 10km có giá trung bình khoảng 2.0 triệu:
# Ta chọn hệ số góc giảm trung bình là 0.25 triệu/km (4.5 - 0.25 * 10 = 2.0 triệu).
# Sai số ngẫu nhiên (noise) tuân theo phân phối chuẩn với độ lệch chuẩn là 0.25 triệu VNĐ.
noise = np.random.normal(0, 0.25, n_samples)
rent_prices = 4.5 - 0.25 * distances + noise

# Làm tròn giá thuê đến 1 chữ số thập phân (ví dụ 3.2 triệu, 2.5 triệu)
rent_prices = np.round(rent_prices, 1)

# Giới hạn giá trọ tối thiểu là 1.0 triệu (không thể âm hoặc quá rẻ)
rent_prices = np.clip(rent_prices, 1.0, 5.5)
